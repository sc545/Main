...
//            }
            ...
            if (rectBubbleArea.left < (bubble.x - bubble.bubbleR) && rectBubbleArea.top < (bubble.y - bubble.bubbleR) && rectBubbleArea.right > (bubble.x + bubble.bubbleR) && rectBubbleArea.bottom > (bubble.y + bubble.bubbleR)) {
                if (!gameStageActivity.m_bFeverState) {
                    if(300 - gameStageActivity.m_nGameTime*2 - score - gameStageActivity.m_nCombo/10> 50) {
                        Thread.sleep(300 - gameStageActivity.m_nGameTime*2 - score - gameStageActivity.m_nCombo/10); // 스레드가 0.5초 동안 잠든다 => 0.5초 간격으로 버블 생성
                        aniNewBubble();
                        arrayList.add(bubble);
                    }else{
                        Thread.sleep(50); // 스레드가 0.5초 동안 잠든다 => 0.5초 간격으로 버블 생성
                        aniNewBubble();
                        arrayList.add(bubble);
                    }
                }else{
